...
const AIAndStockMarketNews = () => {
  const navigate = useNavigate();
  const { theme, getSectionHeaderClasses } = useTheme();
  const { t } = useLanguage();
  const [activeTab, setActiveTab] = useState('ai'); // 'ai' or 'stock-market'
  ...
  useEffect(() => {
    const fetchAIStockMarketData = async () => {
      try {
        setLoading(true);
        ...
        if (stockMarketResponse.ok) {
          ...
          // Fallback: use ai articles for stock market if not available
          if (aiResponse.ok) {
            const aiData = await aiResponse.json();
            setStockMarketArticles(aiData);
          }
        }
        ...
        console.error('Error loading AI and stock market data:', err);
        setError('Failed to load AI and stock market news. Please try again later.');
      } finally {
        setLoading(false);
      }
    };

    fetchAIStockMarketData();
  }, []);

  // Update filtered articles when tab or filter changes
  useEffect(() => {
    const currentArticles = activeTab === 'stock-market' ? stockMarketArticles : aiArticles;
    const filtered = filterArticlesByDate(currentArticles, selectedFilter);
    setFilteredArticles(filtered);
  }, [aiArticles, stockMarketArticles, activeTab, selectedFilter]);
  ...
  const getThumbnail = (index) => {
    const thumbnails = [
      'https://images.unsplash.com/photo-1485827404703-89b55fcc595e?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1518186285589-2f7649de83e0?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1590283603385-17ffb3a7f29f?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1526304640581-d334cdbbf45e?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1639322537228-f710d846310a?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1504868584819-f8e8b4b6d7e3?w=60&h=45&fit=crop',
    ...
    return (
      <div className={`min-h-screen ${themeClasses.pageBackground} flex items-center justify-center`}>
        <div className="text-center">
          ...
          <p className={`text-lg font-medium ${themeClasses.textPrimary}`}>Loading AI & Stock Market News...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`min-h-screen ${themeClasses.pageBackground} flex items-center justify-center`}>
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <h2 className={`text-2xl font-bold ${themeClasses.textPrimary} mb-2`}>Unable to Load AI & Stock Market News</h2>
    ...
    <div className={`min-h-screen ${themeClasses.pageBackground}`}>
      ...
      <div className="max-w-5xl-plus mx-auto px-8 pb-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-8">
          ...
          {/* AI Stock Market News Section - 70% width */}
          <div className="lg:col-span-7 -mt-1">
            {/* AI Stock Market News Section Header - Sticky with filter and bottom border */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              {filteredArticles.length > 0 ? (
                filteredArticles.map((article) => (
                  ...
                  >
                    <div className="flex items-start space-x-3 text-left pr-3">
                      <img
                        src={article.image_url || (activeTab === 'stock-market' ? 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=300&h=200&fit=crop' : 'https://images.unsplash.com/photo-1485827404703-89b55fcc595e?w=300&h=200&fit=crop')}
                        alt={article.title}
                ...
                <div className="col-span-2 text-center py-12">
                  <div className="text-6xl mb-4">{activeTab === 'stock-market' ? '📈' : '🤖'}</div>
                  <h3 className="text-lg font-medium text-gray-900 mb-2">No {activeTab === 'stock-market' ? 'Stock Market' : 'AI'} News Found</h3>
                  <p className="text-gray-600 mb-4">
                    No {activeTab === 'stock-market' ? 'stock market' : 'AI'} articles are available for the selected time period.
                  </p>
...
export default AIAndStockMarketNews;