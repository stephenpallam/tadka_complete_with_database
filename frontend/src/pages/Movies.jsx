...
const Movies = () => {
  ...
  const { t } = useLanguage();
  const [activeTab, setActiveTab] = useState('movies'); // 'movies' or 'bollywood'
  ...
  useEffect(() => {
    const fetchMoviesData = async () => {
      try {
        ...
        // Fetch articles from the backend API using movies and movie-reviews categories
        ...
        try {
          ...
          console.warn('Failed to fetch movies category:', error);
        }
        ...
        try {
          ...
          console.warn('Failed to fetch movie-reviews category:', error);
        }
        
        // Remove duplicates based on ID and sort by published_at descending
        const uniqueMoviesData = combinedMoviesData
        ...
        if (bollywoodResponse.ok) {
          ...
          // Fallback: use combined movies articles for bollywood if not available
          setBollywoodArticles(uniqueMoviesData);
        }
        ...
        setError('Failed to load movies news. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const currentArticles = activeTab === 'bollywood' ? bollywoodArticles : moviesArticles;
    const filtered = filterArticlesByDate(currentArticles, selectedFilter);
    setFilteredArticles(filtered);
  }, [moviesArticles, bollywoodArticles, activeTab, selectedFilter]);
  ...
  const getThumbnail = (index) => {
    const thumbnails = [
      ...
      'https://images.unsplash.com/photo-1518709268805-4e9042af2176?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1534447677768-be436bb09401?w=60&h=45&fit=crop',
      ...
      'https://images.unsplash.com/photo-1606177068212-1d3e4cd0d8be?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1518869175114-83ea80a3b9c1?w=60&h=45&fit=crop',
  ...
  const filterArticlesByDate = (articles, filter) => {
    ...
    const filtered = articles.filter((article) => {
      ...
      switch (filter) {
        case 'thisWeek':
          // This week means Monday to Sunday + previous 2 days for better coverage
          const currentWeekStart = new Date(today);
          const dayOfWeek = today.getDay();
          const daysToMonday = dayOfWeek === 0 ? 6 : dayOfWeek - 1;
          currentWeekStart.setDate(today.getDate() - daysToMonday);
          
          // Extend start date by 2 more days (include previous 2 days)
          const extendedWeekStart = new Date(currentWeekStart);
          extendedWeekStart.setDate(currentWeekStart.getDate() - 2);
          
          // Calculate week end (Sunday)
          const currentWeekEnd = new Date(currentWeekStart);
          currentWeekEnd.setDate(currentWeekStart.getDate() + 6);
          
          return articleDateOnly >= extendedWeekStart && articleDateOnly <= currentWeekEnd;
        case 'today':
    ...
    return (
      <div className={`min-h-screen ${themeClasses.pageBackground} flex items-center justify-center`}>
        <div className="text-center">
          ...
          <p className={`text-lg font-medium ${themeClasses.textPrimary}`}>Loading Movies...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`min-h-screen ${themeClasses.pageBackground} flex items-center justify-center`}>
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <h2 className={`text-2xl font-bold ${themeClasses.textPrimary} mb-2`}>Unable to Load Movies</h2>
    ...
    <div className={`min-h-screen ${themeClasses.pageBackground}`}>
      ...
      <div className="max-w-5xl-plus mx-auto px-8 pb-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-8">
          ...
          {/* Movies Section - 70% width */}
          <div className="lg:col-span-7 -mt-1">
            {/* Movies Section Header - Sticky with filter and bottom border */}
                ...
                >
                  <div className="flex items-start space-x-3 text-left pr-3">
                    <ArticleImage
                      ...
                      alt={article.title}
                      contentType={activeTab === 'bollywood' ? 'bollywood-movies' : 'movies'}
