...
const HotTopicsGossipNews = () => {
  ...
  const [hotTopicsArticles, setHotTopicsArticles] = useState([]);
  ...
  useEffect(() => {
    const fetchHotTopicsGossipData = async () => {
      try {
        setLoading(true);
        
        // Fetch articles from the backend API using hot-topics and gossip categories
        const hotTopicsResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001'}/api/articles/category/hot-topics?limit=50`);
        if (hotTopicsResponse.ok) {
          const hotTopicsData = await hotTopicsResponse.json();
          setHotTopicsArticles(hotTopicsData);
        }

        const gossipResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001'}/api/articles/category/gossip?limit=50`);
        if (gossipResponse.ok) {
          const gossipData = await gossipResponse.json();
          setGossipArticles(gossipData);
        } else {
          // Fallback: use hot topics articles for gossip if not available
          const hotTopicsData = await hotTopicsResponse.json();
          setGossipArticles(hotTopicsData);
        }
        
        // Get related articles from configured categories for hot topics gossip page
        try {
          const response = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001'}/api/related-articles/hot-topics-gossip`);
          if (response.ok) {
        ...
        console.error('Error loading hot topics gossip data:', err);
        setError('Failed to load hot topics and gossip. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const getThumbnail = (index) => {
    const thumbnails = [
      'https://images.unsplash.com/photo-1492633423870-43d1cd2775eb?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1529626455594-4ff0802cfb7e?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1508214751196-bcfd4ca60f91?w=60&h=45&fit=crop',
      ...
      'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1547425260-76bcadfb4f2c?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1484399172022-72a90b12e3c1?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?w=60&h=45&fit=crop'
    ];
    ...
    return (
      <div className={`min-h-screen ${themeClasses.pageBackground} flex items-center justify-center`}>
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <h2 className={`text-2xl font-bold ${themeClasses.textPrimary} mb-2`}>Unable to Load Hot Topics & Gossip</h2>
    ...
    <div className={`min-h-screen ${themeClasses.pageBackground}`}>
      ...
      <div className="max-w-5xl-plus mx-auto px-8 pb-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-8">
          ...
          {/* Hot Topics & Gossip Section - 70% width */}
          <div className="lg:col-span-7 -mt-1">
            {/* Hot Topics & Gossip Section Header - Sticky with filter and bottom border */}
              ...
              {filteredArticles.map((article) => (
                ...
                >
                  <div className="flex items-start space-x-3 text-left pr-3">
                    <img
                      src={article.image_url || 'https://images.unsplash.com/photo-1492633423870-43d1cd2775eb?w=300&h=200&fit=crop'}
                      alt={article.title}
...
export default HotTopicsGossipNews;