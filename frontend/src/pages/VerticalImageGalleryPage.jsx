...
const VerticalImageGalleryPage = () => {
  ...
  // Vertical gallery images (portrait style)
  const verticalGalleryImages = [
    {
      id: 1,
      fullImage: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=400&h=600&fit=crop',
      name: 'Tech Entrepreneur Portrait',
      isVertical: true
    },
    {
      id: 2,
      fullImage: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=400&h=600&fit=crop',
      name: 'Business Leader Profile',
      isVertical: true
    },
    {
      id: 3,
      fullImage: 'https://images.unsplash.com/photo-1560250097-0b93528c311a?w=400&h=600&fit=crop',
      name: 'Young CEO Portrait',
      isVertical: true
    },
    {
      id: 4,
      fullImage: 'https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?w=400&h=600&fit=crop',
      name: 'Startup Founder',
      isVertical: true
    },
    {
      id: 5,
      fullImage: 'https://images.unsplash.com/photo-1566492031773-4f4e44671d66?w=400&h=600&fit=crop',
  ...
  useEffect(() => {
    const loadGalleryData = async () => {
      try {
        ...
        // Create gallery object (for the entrepreneur profile)
        const gallery = {
          id: parseInt(id),
          title: 'Profile: Young Entrepreneur\'s Success Journey',
          summary: 'This professional portrait gallery showcases successful entrepreneurs and business leaders who are making their mark in various industries. Our photography team has captured these inspiring individuals in their element, highlighting their achievements and professional excellence.',
          content: 'Each portrait tells a story of determination, innovation, and success in the modern business landscape. These images represent the new generation of leaders who are shaping the future of entrepreneurship and inspiring others to pursue their dreams with confidence and vision.',
          category: 'Business',
          author: 'DesiTrends Business Team',
          published_at: new Date().toISOString(),
          view_count: Math.floor(Math.random() * 2000) + 300,
          section: 'vertical_image_gallery'
        };
        ...
        for (let i = 1; i <= 8; i++) {
          if (i !== parseInt(id)) {
            related.push({
              id: i,
              title: `Portrait Collection ${i}`,
              summary: `Professional portrait gallery ${i}`
            });
          }
        }
  ...
  useEffect(() => {
    ...
    if (metaDescription) {
      metaDescription.content = currentGallery.summary || 'Professional entrepreneur portraits on DesiTrends.';
    }
    ...
    const structuredData = {
      ...
      "author": {
        "@type": "Person",
        "name": currentGallery.author || "DesiTrends Business Team"
      },
      "publisher": {
        "@type": "Organization",
        "name": "DesiTrends",
    ...
    const existingStructuredData = document.querySelector('#vertical-gallery-structured-data');
    if (existingStructuredData) {
      existingStructuredData.remove();
    }

    const script = document.createElement('script');
    script.id = 'vertical-gallery-structured-data';
    ...
    // Cleanup
    return () => {
      const structuredDataElement = document.querySelector('#vertical-gallery-structured-data');
  ...
  const updateOpenGraphTags = () => {
    ...
    const ogTags = [
      ...
      { property: 'og:description', content: currentGallery.summary || 'Professional entrepreneur portraits on DesiTrends' },
  ...
  const trackGalleryView = async () => {
    try {
      const trackingData = {
        ...
        section: 'vertical_image_gallery_page',
        action: 'vertical_gallery_page_view',
        timestamp: new Date().toISOString(),
        userAgent: navigator.userAgent,
        url: window.location.href,
        referrer: document.referrer,
        source: 'vertical_gallery_page',
        viewport: `${window.innerWidth}x${window.innerHeight}`,
        deviceType: isMobile ? 'mobile' : 'desktop',
        engagement: 'high',
        contentType: 'business_portraits',
  ...
  const handlePrevImage = () => {
    const newIndex = currentImageIndex === 0 ? verticalGalleryImages.length - 1 : currentImageIndex - 1;
    setCurrentImageIndex(newIndex);
    trackImageNavigation('previous', newIndex);
  };

  const handleNextImage = () => {
    const newIndex = (currentImageIndex + 1) % verticalGalleryImages.length;
    setCurrentImageIndex(newIndex);
  ...
  const trackImageNavigation = async (direction, imageIndex) => {
    try {
      const trackingData = {
        ...
        imageName: verticalGalleryImages[imageIndex].name,
        action: `vertical_image_navigation_${direction}`,
        timestamp: new Date().toISOString(),
        source: 'vertical_gallery_page'
      };
  ...
  const handleRelatedGalleryClick = async (relatedGallery) => {
    try {
      ...
      const trackingData = {
        ...
        action: 'related_vertical_gallery_click',
        timestamp: new Date().toISOString(),
        source: 'vertical_gallery_page_related'
      };
      ...
      navigate(`/vertical-gallery/${relatedGallery.id}`);
  ...
  const getThumbnail = (index) => {
    const thumbnails = [
      'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1560250097-0b93528c311a?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1566492031773-4f4e44671d66?w=60&h=45&fit=crop',
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg font-medium text-gray-800">Loading Portrait Gallery</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-white flex items-start justify-start pt-20">
        <div className="ml-8 bg-white border border-gray-300 p-6 max-w-md">
          ...
          <h2 className="text-lg font-semibold text-gray-900 mb-2">Portrait gallery not found</h2>
          <p className="text-gray-600 mb-4 text-sm">The portrait gallery you're looking for doesn't exist or has been removed.</p>
          <button 
            onClick={handleBack}
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-5xl mx-auto px-8 py-6">
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            >
              ...
              <div className="relative">
                <img
                  ...
                  style={{ width: '348px', height: '420px' }}
          ...
          {/* Related Portraits Sidebar - 35% width */}
      ...
      {imageModalOpen && selectedImage && (
        <div style={{ zIndex: 9999 }}>
          <ImageModal
            ...
            onNext={() => {
              const nextIndex = (currentImageIndex + 1) % verticalGalleryImages.length;
              setCurrentImageIndex(nextIndex);
              setSelectedImage(verticalGalleryImages[nextIndex]);
            }}
            onPrev={() => {
              const prevIndex = currentImageIndex === 0 ? verticalGalleryImages.length - 1 : currentImageIndex - 1;
              setCurrentImageIndex(prevIndex);
              setSelectedImage(verticalGalleryImages[prevIndex]);
            }}
          />
        </div>
      )}
...
export default VerticalImageGalleryPage;