...
const Sports = () => {
  ...
  const { t } = useLanguage();
  const [activeTab, setActiveTab] = useState('cricket'); // 'cricket' or 'other-sports'
  const [cricketArticles, setCricketArticles] = useState([]);
  const [otherSportsArticles, setOtherSportsArticles] = useState([]);
    ...
    const fetchSportsData = async () => {
      try {
        ...
        if (sportsResponse.ok) {
          ...
          // Fallback: use cricket articles for other sports if not available
          const cricketData = await cricketResponse.json();
          setOtherSportsArticles(cricketData);
        }
        
        // Get related articles from configured categories for sports page
        try {
          const response = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001'}/api/related-articles/sports`);
          if (response.ok) {
  ...
  useEffect(() => {
    const currentArticles = activeTab === 'other-sports' ? otherSportsArticles : cricketArticles;
    const filtered = filterArticlesByDate(currentArticles, selectedFilter);
    setFilteredArticles(filtered);
  }, [cricketArticles, otherSportsArticles, activeTab, selectedFilter]);
  ...
  const getThumbnail = (index) => {
    const thumbnails = [
      ...
      'https://images.unsplash.com/photo-1593766827883-2db900031e9d?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1574629810360-7efbbe195018?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1551698618-1dfe5d97d256?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1560272564-c83b66b1ad12?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1594736797933-d0c43012b7ab?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1546525848-3ce03ca516f6?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1579952363873-27d3bfad9c0d?w=60&h=45&fit=crop',
    ...
    return (
      <div className={`min-h-screen ${themeClasses.pageBackground} flex items-center justify-center`}>
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <h2 className={`text-2xl font-bold ${themeClasses.textPrimary} mb-2`}>Unable to Load Sports</h2>
    ...
    <div className={`min-h-screen ${themeClasses.pageBackground}`}>
      ...
      <div className="max-w-5xl-plus mx-auto px-8 pb-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-8">
          ...
          {/* Sports Section - 70% width */}
          <div className="lg:col-span-7 -mt-1">
            {/* Sports Section Header - Sticky with filter and bottom border */}
              ...
              <div className="pl-0 pr-4 py-4">
                <div className="mb-2">
                  ...
                  <div className="flex space-x-4">
                    ...
                    <button
                      onClick={() => setActiveTab('other-sports')}
                      className={`text-base font-bold transition-colors duration-200 ${
                        activeTab === 'other-sports'
                          ? 'text-black'
                          : 'text-gray-600 hover:text-gray-800'
                      }`}
                    >
              ...
              {filteredArticles.map((article) => (
                ...
                >
                  <div className="flex items-start space-x-3 text-left pr-3">
                    <img
                      src={article.image_url || 'https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=300&h=200&fit=crop'}
                      alt={article.title}
