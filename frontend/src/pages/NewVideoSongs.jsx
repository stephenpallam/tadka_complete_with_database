...
const NewVideoSongs = () => {
  ...
  const { t } = useLanguage();
  const [activeTab, setActiveTab] = useState('video-songs'); // 'video-songs' or 'bollywood'
  const [videoSongsArticles, setVideoSongsArticles] = useState([]);
  ...
  useEffect(() => {
    const fetchNewVideoSongsData = async () => {
      try {
        setLoading(true);
        ...
        if (newVideoSongsResponse.ok) {
          ...
          const videoSongsResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001'}/api/articles/category/new-video-songs?limit=50`);
          if (videoSongsResponse.ok) {
            const videoSongsData = await videoSongsResponse.json();
            setVideoSongsArticles(videoSongsData);
          }
        ...
        // Get related articles for new-video-songs page
        try {
          const response = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001'}/api/related-articles/new-video-songs`);
          if (response.ok) {
        ...
        console.error('Error loading new video songs data:', err);
        setError('Failed to load new video songs. Please try again later.');
      } finally {
        setLoading(false);
      }
    };

    fetchNewVideoSongsData();
  }, []);

  // Update filtered articles when tab or filter changes
  useEffect(() => {
    const currentArticles = activeTab === 'bollywood' ? bollywoodArticles : videoSongsArticles;
    const filtered = filterArticlesByDate(currentArticles, selectedFilter);
    setFilteredArticles(filtered);
  }, [videoSongsArticles, bollywoodArticles, activeTab, selectedFilter]);
  ...
  const getThumbnail = (index) => {
    const thumbnails = [
      ...
      'https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=60&h=45&fit=crop',
      ...
      'https://images.unsplash.com/photo-1578496479914-7ef3b0193be3?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1560169897-fc0cdbdfa4d5?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1594736797933-d0d8e4b15d0a?w=60&h=45&fit=crop',
    ...
    return (
      <div className={`min-h-screen ${themeClasses.pageBackground} flex items-center justify-center`}>
        <div className="text-center">
          ...
          <p className={`text-lg font-medium ${themeClasses.textPrimary}`}>Loading New Video Songs...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`min-h-screen ${themeClasses.pageBackground} flex items-center justify-center`}>
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <h2 className={`text-2xl font-bold ${themeClasses.textPrimary} mb-2`}>Unable to Load New Video Songs</h2>
    ...
    <div className={`min-h-screen ${themeClasses.pageBackground}`}>
      ...
      <div className="max-w-5xl-plus mx-auto px-8 pb-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-8">
          ...
          {/* New Video Songs Section - 70% width */}
          <div className="lg:col-span-7 -mt-1">
            {/* New Video Songs Section Header - Sticky with filter and bottom border */}
              ...
              {filteredArticles.map((article) => (
                ...
                >
                  <div className="flex items-start space-x-3 text-left pr-3">
                    <img
                      src={article.image_url || 'https://images.unsplash.com/photo-1493225457124-a3eb161ffa5f?w=300&h=200&fit=crop'}
                      alt={article.title}
