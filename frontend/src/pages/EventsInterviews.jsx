  ...
  useEffect(() => {
    const fetchEventsInterviewsData = async () => {
      try {
        ...
        const eventsInterviewsResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001'}/api/articles/category/events-interviews?limit=50`);
    ...
    return (
      <div className={`min-h-screen ${themeClasses.pageBackground} flex items-center justify-center`}>
        <div className="text-center">
          ...
          <p className={`text-lg font-medium ${themeClasses.textPrimary}`}>Loading Events & Interviews...</p>
        </div>
      </div>
    );
  }
