...
const Reviews = () => {
  ...
  const { t } = useLanguage();
  const [activeTab, setActiveTab] = useState('movie-reviews'); // 'movie-reviews' or 'ott-reviews'
  const [movieReviews, setMovieReviews] = useState([]);
  const [ottReviews, setOttReviews] = useState([]);
    ...
    const fetchReviewsData = async () => {
      try {
        ...
        if (ottReviewsResponse.ok) {
          ...
          // Fallback: use movie reviews for OTT reviews if not available
          const movieReviewsData = await movieReviewsResponse.json();
          setOttReviews(movieReviewsData);
        }
  ...
  useEffect(() => {
    const currentArticles = activeTab === 'ott-reviews' ? ottReviews : movieReviews;
    const filtered = filterArticlesByDate(currentArticles, selectedFilter);
    setFilteredArticles(filtered);
  }, [movieReviews, ottReviews, activeTab, selectedFilter]);
    ...
    return (
      <div className={`min-h-screen ${themeClasses.pageBackground} flex items-center justify-center`}>
        <div className="text-center">
          ...
          <p className={`text-lg font-medium ${themeClasses.textPrimary}`}>Loading Reviews...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`min-h-screen ${themeClasses.pageBackground} flex items-center justify-center`}>
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <h2 className={`text-2xl font-bold ${themeClasses.textPrimary} mb-2`}>Unable to Load Reviews</h2>
    ...
    <div className={`min-h-screen ${themeClasses.pageBackground}`}>
      ...
      <div className="max-w-5xl-plus mx-auto px-8 pb-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-8">
          ...
          {/* Reviews Section - 70% width */}
          <div className="lg:col-span-7 -mt-1">
            {/* Reviews Section Header - Sticky with filter and bottom border */}
              ...
              <div className="pl-0 pr-4 py-4">
                <div className="mb-2">
                  ...
                  <div className="flex space-x-4">
                    ...
                    <button
                      onClick={() => setActiveTab('ott-reviews')}
                      className={`text-base font-bold transition-colors duration-200 ${
                        activeTab === 'ott-reviews'
                          ? 'text-black'
                          : 'text-gray-600 hover:text-gray-800'
                      }`}
                    >
              ...
              {filteredArticles.map((article) => (
                ...
                >
                  <div className="flex items-start space-x-3 text-left pr-3">
                    <img
                      src={article.image_url || article.image || 'https://images.unsplash.com/photo-1489599112477-990c2cb2c508?w=300&h=200&fit=crop'}
                      alt={article.title}
                    ...
                    <div className="flex-1 min-w-0 text-left">
                      ...
                      {article.rating && (
                        <div className="flex items-center mb-1">
                          <span className="text-xs font-medium text-yellow-600 mr-1">★</span>
                      ...
                      <div className="text-xs text-gray-500 text-left">
                        ...
                        {(article.reviewer || article.author) && (
                          <p className="text-xs text-gray-400">
                            By {article.reviewer || article.author}
                          </p>
                        )}
                      </div>
                    </div>
