...
const TravelPics = () => {
  ...
  const { t } = useLanguage();
  const [activeTab, setActiveTab] = useState('destinations'); // 'destinations' or 'adventures'
  const [destinationsArticles, setDestinationsArticles] = useState([]);
  const [adventuresArticles, setAdventuresArticles] = useState([]);
  ...
  useEffect(() => {
    const fetchTravelPicsData = async () => {
      try {
        setLoading(true);
        
        // Fetch articles from the backend API using travel categories
        const destinationsResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001'}/api/articles/category/travel?limit=50`);
        if (destinationsResponse.ok) {
          const destinationsData = await destinationsResponse.json();
          setDestinationsArticles(destinationsData);
        }

        const adventuresResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001'}/api/articles/category/adventures?limit=50`);
        if (adventuresResponse.ok) {
          const adventuresData = await adventuresResponse.json();
          setAdventuresArticles(adventuresData);
        } else {
          // Fallback: use destinations articles for adventures if not available
          const destinationsData = await destinationsResponse.json();
          setAdventuresArticles(destinationsData);
        }
        
        // Get related articles from configured categories for travel pics page
        try {
          const response = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001'}/api/related-articles/travel-pics`);
          if (response.ok) {
        ...
        console.error('Error loading travel pics data:', err);
        setError('Failed to load travel pics. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const currentArticles = activeTab === 'adventures' ? adventuresArticles : destinationsArticles;
    const filtered = filterArticlesByDate(currentArticles, selectedFilter);
    setFilteredArticles(filtered);
  }, [destinationsArticles, adventuresArticles, activeTab, selectedFilter]);
  ...
  const getThumbnail = (index) => {
    const thumbnails = [
      ...
      'https://images.unsplash.com/photo-1539650116574-75c0c6d73f6e?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1501594907352-04cda38ebc29?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=60&h=45&fit=crop',
    ...
    return (
      <div className={`min-h-screen ${themeClasses.pageBackground} flex items-center justify-center`}>
        <div className="text-center">
          ...
          <p className={`text-lg font-medium ${themeClasses.textPrimary}`}>Loading Travel Pics...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`min-h-screen ${themeClasses.pageBackground} flex items-center justify-center`}>
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <h2 className={`text-2xl font-bold ${themeClasses.textPrimary} mb-2`}>Unable to Load Travel Pics</h2>
    ...
    <div className={`min-h-screen ${themeClasses.pageBackground}`}>
      ...
      <div className="max-w-5xl-plus mx-auto px-8 pb-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-8">
          ...
          {/* Travel Pics Section - 70% width */}
          <div className="lg:col-span-7 -mt-1">
            {/* Travel Pics Section Header - Sticky with filter and bottom border */}
              ...
              {filteredArticles.map((article) => (
                ...
                >
                  <div className="flex items-start space-x-3 text-left pr-3">
                    <img
                      src={article.image_url || 'https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=300&h=200&fit=crop'}
                      alt={article.title}
