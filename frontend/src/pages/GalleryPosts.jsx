...
const GalleryPosts = () => {
  ...
  const { t } = useLanguage();
  const [activeTab, setActiveTab] = useState('travel-pics'); // 'travel-pics' or 'photoshoot'
  const [travelPicsArticles, setTravelPicsArticles] = useState([]);
  const [photoshootArticles, setPhotoshootArticles] = useState([]);
  ...
  useEffect(() => {
    const fetchGalleryPostsData = async () => {
      try {
        setLoading(true);
        
        // Fetch articles from the backend API using travel-pics and photoshoot categories
        const travelPicsResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001'}/api/articles/category/travel-pics?limit=50`);
        if (travelPicsResponse.ok) {
          const travelPicsData = await travelPicsResponse.json();
          setTravelPicsArticles(travelPicsData);
        }

        const photoshootResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001'}/api/articles/category/photoshoot?limit=50`);
        if (photoshootResponse.ok) {
          const photoshootData = await photoshootResponse.json();
          setPhotoshootArticles(photoshootData);
        } else {
          // Fallback: use travel pics articles for photoshoot if not available
          const travelPicsData = await travelPicsResponse.json();
          setPhotoshootArticles(travelPicsData);
        }
        
        // Get related articles from configured categories for gallery posts page
        try {
          const response = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001'}/api/related-articles/gallery-posts`);
          if (response.ok) {
        ...
        console.error('Error loading gallery posts data:', err);
        setError('Failed to load gallery posts. Please try again later.');
      } finally {
        setLoading(false);
      }
    };

    fetchGalleryPostsData();
  }, []);

  // Update filtered articles when tab or filter changes
  useEffect(() => {
    const currentArticles = activeTab === 'photoshoot' ? photoshootArticles : travelPicsArticles;
    const filtered = filterArticlesByDate(currentArticles, selectedFilter);
    setFilteredArticles(filtered);
  }, [travelPicsArticles, photoshootArticles, activeTab, selectedFilter]);
  ...
  const getThumbnail = (index) => {
    const thumbnails = [
      ...
      'https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1586339949216-35c2747cb8de?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1573164713714-d95e436ab8d6?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1611224923853-80b023f02d71?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1585776245991-cf89dd7fc73a?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1526666923127-b2970f64b422?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1549880338-65ddcdfd017b?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1501594907352-04cda38ebc29?w=60&h=45&fit=crop',
      'https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=60&h=45&fit=crop',
  ...
  const handleRelatedArticleClick = (article) => {
    // Navigate to individual gallery post page
    ...
    return (
      <div className={`min-h-screen ${themeClasses.pageBackground} flex items-center justify-center`}>
        <div className="text-center">
          ...
          <p className={`text-lg font-medium ${themeClasses.textPrimary}`}>Loading Gallery Posts...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`min-h-screen ${themeClasses.pageBackground} flex items-center justify-center`}>
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <h2 className={`text-2xl font-bold ${themeClasses.textPrimary} mb-2`}>Unable to Load Gallery Posts</h2>
    ...
    <div className={`min-h-screen ${themeClasses.pageBackground}`}>
      ...
      <div className="max-w-5xl-plus mx-auto px-8 pb-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-8">
          ...
          {/* Gallery Posts Section - 70% width */}
          <div className="lg:col-span-7 -mt-1">
            {/* Gallery Posts Section Header - Sticky with filter and bottom border */}
              ...
              {filteredArticles.map((article) => (
                ...
                >
                  <div className="flex items-start space-x-3 text-left pr-3">
                    <img
                      src={article.image_url || 'https://images.unsplash.com/photo-1495020689067-958852a7765e?w=300&h=200&fit=crop'}
                      alt={article.title}
